...
void USteamUtils::ConvertIPStringToUint32(const FString& IP, uint32& OutIP)
{
...
FString USteamUtils::ConvertIPToString(uint32 IP)
{
	return FString::FromInt(IP >> 24) + "." + FString::FromInt((IP >> 16) % 256) + "." + FString::FromInt((IP >> 8) % 256) + "." + FString::FromInt(IP % 256);
}

FString USteamUtils::GetSteamIDAsString(const FSteamID& SteamID)
{
